...
int batadv_mesh_init(struct net_device *soft_iface)
{
	...
	batadv_hopmode_init(bat_priv); //Hmmm
...
void batadv_mesh_free(struct net_device *soft_iface)
{
	...
	batadv_hopmode_free(bat_priv); // Hmmm
			   ...
			   struct net_device *orig_dev)
{
	...
	batadv_inc_counter(bat_priv, BATADV_CNT_TOTAL_DETECTED);
	//Collisiooooooon
	if(!!atomic_read(&bat_priv->collision_mode)){
		arrival_time=ktime_get();
		//pack_ts=skb->tstamp;

		if(bat_priv->last_packet_time==KTIME_INVALID){
			bat_priv->last_packet_time=arrival_time;
		}
		else{
			difference=ktime_sub(arrival_time,bat_priv->last_packet_time);
			/*
			if(bat_priv->min_diff==KTIME_INVALID){
				bat_priv->min_diff=difference;
				bat_priv->max_diff=difference;
			}
			else{
				if(difference < bat_priv->min_diff )
					bat_priv->min_diff=difference;
				if(bat_priv->max_diff < difference )
					bat_priv->max_diff=difference;
			}
			*/

			bat_priv->last_difference=difference;
			bat_priv->last_packet_time=arrival_time;
			//Collision ile ilgili istatistikleri unutma.




			//Buraya collision counter'ini da katmamiz gerekiyor.

			//Difference ne kadar kucukse o kadar yogun trafik var demek,
			//belli bir threshold'dan daha kucukse artik collision olmus demektir.
			
			//Threshold ne kadar kucukse difference'in threshold'dan daha kucuk olmasi
			//o kadar zor olur yani kucuk threshold demek daha rahat ve güvenli bir ortam demek.
			if(bat_priv->collision_threshold!=KTIME_INVALID && (difference < bat_priv->collision_threshold)){
				batadv_dbg(BATADV_DBG_BATMAN, bat_priv,
			   		"Collision happened!\n");
			   	batadv_inc_counter(bat_priv, BATADV_CNT_COLLISION);
			   	goto err_free;
			}
		}
	}
