...
static const struct nla_policy batadv_netlink_policy[NUM_BATADV_ATTR] = {
	//[BATADV_ATTR_SOMETHING]			= { .type = NLA_U8 },
	[BATADV_ATTR_COLLMODE_THRESHOLD]		= { .type = NLA_S64 },
	[BATADV_ATTR_COLLMODE_MODE]				= { .type = NLA_U8 },
	[BATADV_ATTR_COLLMODE_LAST_DIFFERENCE]	= { .type = NLA_S64 },
	[BATADV_ATTR_COLLMODE_MIN_DIFF]			= { .type = NLA_S64 },
	[BATADV_ATTR_COLLMODE_MAX_DIFF]			= { .type = NLA_S64 },
	[BATADV_ATTR_COLLMODE_RESET_STAT]		= { .type = NLA_U8 },

	[BATADV_ATTR_HOPMODE_COMMAND]		= { .type = NLA_U8 },
	[BATADV_ATTR_HOPMODE_PENALTY]		= { .type = NLA_U16 },
				    ...
				    u32 portid, u32 seq, int flags)
{
	...
	//Collision ile ilgili seyler
	if (nla_put_u8(msg, BATADV_ATTR_COLLMODE_MODE,
			!!atomic_read(&bat_priv->collision_mode)) || 
		nla_put_s64(msg, BATADV_ATTR_COLLMODE_THRESHOLD,
			bat_priv->collision_threshold,BATADV_ATTR_PAD) ||  
		nla_put_s64(msg, BATADV_ATTR_COLLMODE_LAST_DIFFERENCE,
			bat_priv->last_difference,BATADV_ATTR_PAD) /*||
		nla_put_s64(msg, BATADV_ATTR_COLLMODE_MIN_DIFF,
			bat_priv->min_diff,BATADV_ATTR_PAD) ||
		nla_put_s64(msg, BATADV_ATTR_COLLMODE_MAX_DIFF,
			bat_priv->max_diff,BATADV_ATTR_PAD)*/
		)
		goto nla_put_failure;
...
static int batadv_netlink_set_mesh(struct sk_buff *skb, struct genl_info *info)
{
	...
	//Related to the collision mode
	if (info->attrs[BATADV_ATTR_COLLMODE_THRESHOLD]) {
		s64 thresh;

		attr = info->attrs[BATADV_ATTR_COLLMODE_THRESHOLD];
		thresh = nla_get_s64(attr);
		//printk("%lld is the threshold value.\n",thresh);
		bat_priv->collision_threshold=thresh;
	}

	if (info->attrs[BATADV_ATTR_COLLMODE_RESET_STAT]) {
		u8 reset;

		attr = info->attrs[BATADV_ATTR_COLLMODE_RESET_STAT];
		reset = nla_get_u8(attr);

		if(reset){
			batadv_reset_counter(bat_priv,BATADV_CNT_COLLISION);
			batadv_reset_counter(bat_priv,BATADV_CNT_TOTAL_DETECTED);
			bat_priv->last_packet_time=KTIME_INVALID;
			bat_priv->last_difference=KTIME_INVALID;
			bat_priv->collision_threshold=KTIME_INVALID;
			//bat_priv->min_diff=KTIME_INVALID;
			//bat_priv->max_diff=KTIME_INVALID;
		}
	}

	if (info->attrs[BATADV_ATTR_COLLMODE_MODE]) {
		u8 mode;

		attr = info->attrs[BATADV_ATTR_COLLMODE_MODE];
		mode = nla_get_u8(attr);

		atomic_set(&bat_priv->collision_mode,!!mode);
	}
...
static int batadv_netlink_hopmode(struct sk_buff *skb, struct genl_info *info)
{
	struct batadv_priv *bat_priv = info->user_ptr[0];
	struct nlattr *attr;
	u16 neigh_penal=0;
	...
	if (info->attrs[BATADV_ATTR_HOPMODE_PENALTY]) {
		attr = info->attrs[BATADV_ATTR_HOPMODE_PENALTY];
		neigh_penal=nla_get_u16(attr);
	}
	
	if (info->attrs[BATADV_ATTR_HOPMODE_COMMAND]) {
		attr = info->attrs[BATADV_ATTR_HOPMODE_COMMAND];
		hopmode_command=nla_get_u8(attr);
		//pr_info("Someone from userspace in this node said the number: %d\n",sth);
		batadv_hopmode_tvlv_container_update(bat_priv,neigh_penal,hopmode_command);
	}
...
static const struct genl_small_ops batadv_netlink_ops[] = {
	...
	{
		.cmd = BATADV_CMD_HOPMODE,
		.validate = GENL_DONT_VALIDATE_STRICT | GENL_DONT_VALIDATE_DUMP,
		.flags = GENL_UNS_ADMIN_PERM,
		.doit = batadv_netlink_hopmode,
		.internal_flags = BATADV_FLAG_NEED_MESH,
	},
